...
library(dplyr) #Cargar paquete, si no estÃ¡ cargado desde antes.
...
MinGibbsFreeEnergyUncertainty <- function(Min, Tk,T0)    #Calculates uncertainties of minerals
{
    ...
    if ( !(is.na( CEP$SD..cal.mol.1)))
  {
    ...
    if ( !(is.na(CEP$S..cal.mol.1.Â.K.1)) )
   {oS<-as.numeric(CEP$S..cal.mol.1.Â.K.1);} else {oS<-0;}
    if( !(is.na(CEP$SD..cal.mol.1.Â.K.1)))
   {oV<-as.numeric(CEP$SD..cal.mol.1.Â.K.1)} else {oV<-0;}
    ...
    if (!(is.na(CEP$a..cal.mol.1.Â.K.1 )))
    {oc<-as.numeric(CEP$a..cal.mol.1.Â.K.1 );} else {oc<-0;}
...
SolvFunc <- function(p, TC, Pbar)           #Solvent function (g), Shock et al , 1992; Sue et al , 2002 - valid at Ï>=0.20 and <=1, Ï>=0.35 - Shock et al., 1992 (p. 809, Fig. 6)
{
g<-0;
ag<-0;
bg<-0;
p_a<-0; 
ag_i = -2.037662;         #Ã…    Shock et al., 1992 (Table 3, p. 807)
ag_ii = 0.005747;         #Ã… Â°k     Shock et al., 1992 (Table 3, p. 807)
ag_iii = -0.000006557892; #Ã… Â°k^2  Shock et al., 1992 (Table 3, p. 807)
bg_i = 6.107361;          #         Shock et al., 1992 (Table 3, p. 807)
bg_ii = -0.01074377;      #1 Â°C    Shock et al., 1992 (Table 3, p. 807)
bg_iii = 0.00001268348;   #1 Â°C^2  Shock et al., 1992 (Table 3, p. 807)
ag_1 = 3.66666E-16;       #Â°k       Shock et al., 1992 (Table 4, p. 809)
ag_2 = -1.504956E-10;     #Ã… bar^3  Shock et al., 1992 (Table 4, p. 809)
ag_3 = 5.01799E-14;       #Ã… bar^4  Shock et al., 1992 (Table 4, p. 809)
...
if (p_a>1 )
{ 
p_a<-1;                                         #//Need T0 avoid the calculation collapse at T>25Â°C and provide some minor error on the Born function
...
{
  g<-g-(cg_1*p_a+cg_2* (p_a^2)+cg_3*log(p_a));             #Sue et al., 2002 (Eq. 25, p. 3304) - valid at T - from 250 T0 600Â°C, Pbar - from 223 T0 998 bar, Ï - from 0.20 T0 0.81 g cm-3 (p. 3303)
}
...
DielConst <- function(p, TC, Tk, Pbar)                      #//Dielectric constant ofwater ( e) at specified T and P
{
  ...
  a_1 <- -1.576377E-3;    #1/Â°C                //Sverjensky et al., 2014 (Table 1, p. 131)
  a_2 <- 6.810288E-2;     #1/sqrt(Â°C)         //Sverjensky et al., 2014 (Table 1, p. 131)
  a_3 <- 7.548755E-1;     #                    //Sverjensky et al., 2014 (Table 1, p. 131)
  b_1 <- -8.016651E-5;    #1/Â°C                //Sverjensky et al., 2014 (Table 1, p. 131)
  b_2 <- -6.871618E-2;   #1/sqrt(Â°C)          //Sverjensky et al., 2014 (Table 1, p. 131)
...
#View(CEP) #VisualizaciÃ³n de la base

G<-as.numeric(CEP$G..cal.mol.1);
S<- as.numeric(SD..cal.mol.1);


V<- as.numeric(CEP$S..cal.mol.1.Â.K.1);

a<- as.numeric(CEP$SD..cal.mol.1.Â.K.1);
...
#other functions
 waterGibbsFreeEnergy<- function(Tk,Pbar,p) #Calculates the Gibbs energy ofwater from triple point Ptr and Ttr T0 10 kbar and 1273.15 Â°k (Helgeson and kirkham, 1974; Hill, 1990; Johnson and NorT0n, 1991; Han, 2008)
{ 
...
#const
MH2O <- 18.015268;
Tcr <- 647.067; # {Â°k}      #critical temperature ofwater from (Johnson and NorT0n, 1991),  Tcr = 647.096Â°k in http://en. wikipedia.org/ wiki/Critical_point_(thermodynamics)
Ttr <- 273.16;  # {Â°k}      #temperature at the triple point
...
SH2O<-Rcal*(T*kT)+SH2Otr;     #{cal/Â°k/mol}                          #Entropy ofwater, Johnson and NorT0n, 1991 (p. 586)
...
Born<- function( w, charge, Tk, Pbar, gH2O,  eTP,T0)     #//Calculates the Gibbs free energies of solvation
{
  ...
  Y = -5.8495398E-5;     #{1/Â°k}                //Shock et al., 1992 (Appendix D, p. 824)
  n = 166027;            #{Ã…*cal/mol}           // n=NAv*e*e/2 (e=4.80298E-10 {cm^1.5*g^0.5/s} - electronic charge, Shock et al., 1992 (p. 803, Appendix A, p. 818)
  e = 78.38092765;       #{}                    //Dielectric constant ofwater at T = 298.15Â°k and P = 1 bar
  kz = 0.94;             #{Ã…}                   //Constant for cations, Shock et al., 1992 (p. 805)
  
  if (!(charge==0))                         #//Born coefficient of the ion, Shock et al., 1992 (p. 803-805, Appendix D, p. 824)
  { 
    re<-charge*charge/( w*100000/ n+charge/3.082);   #//Effective electroctatic radii of ions, 3.082 corresponds T0 the effective electrostatic radius of H+ at 1 bar and 298.15Â°k}
...
AqGibbsFreeEnergy<- function(Aq,Tk, Pbar, gH2O,  eTP)     #//Calculates Gibbs Free energies of aqueous species
{
 ...
 w<-0; 
  charge<-0;
  theta = 228; #{Â°k}   //Solvent constant, Shock et al., 1992 (Appendix A-B, pp. 819-820)
...
if(! ( CEP$Name == Aq))   #//Find existence of the aqueous species in the data base
{
...
else
 {
  ...
  if (!(is.na( CEP$S..cal.mol.1.Â.K.1)))
  {
    oS<- as.numeric(CEP$S..cal.mol.1.Â.K.1)  ;
  }
  else
  {
    oS<-0;
  ...
  if (!(is.na( CEP $a..cal.mol.1.Â.K.1 ) ))
   { 
    oa1<- as.numeric(CEP $a..cal.mol.1.Â.K.1 );
   }
  else 
  {
    oa1<-0;
  }
  if(!(is.na(CEP $bÂ.1000..cal.mol.1.Â.K.1 )) )  
  {
    oa2<- as.numeric(CEP $bÂ.1000..cal.mol.1.Â.K.1 ) ;
    } 
  else
    {
    oa2<-0;
    }
  if (!(is.na(CEP $cÂ.10.5..cal.mol.1.Â.K.1)) )
  {
    oa3<- as.numeric(CEP $cÂ.10.5..cal.mol.1.Â.K.1) ;
   }
  else
  { 
  oa3<-0;
...
Result<-sqrt( (oG^2)+ (oS*(Tk-T0)^2)+ (oc1*(Tk-T0-Tk*log10(Tk/T0))^2)+ (oa1/10*(Pbar-1)^2)+ (oa2*100*log10((psi+Pbar)/(psi+1))^2)+ (oc2*10000*((1/(Tk-theta)-1/(T0-theta))*(theta-Tk)/theta-Tk/theta/theta*log10(T0*(Tk-theta)/Tk/70.15))^2)+ (oa3*(Pbar-1)/(Tk-theta)^2)+ (oa4*10000*log10((psi+Pbar)/(psi+1))/(Tk-theta)^2));    #//+ (o w*100000*((1-1/epsilon)+Y*(Tk-T0)),2));
 ...
 GasGibbsFreeEnergy<-function(Gas,Tk,Pbar)                #//Calculates Gibbs Free energies of gas species
 {
   ...
   #View(CEP) #VisualizaciÃ³n de la base
       ...
       G<- as.numeric(CEP$G..cal.mol.1) ;
   S<- as.numeric(CEP$S..cal.mol.1.Â.K.1) ;
   a<- as.numeric(CEP $a..cal.mol.1.Â.K.1 ) ;
    b<- as.numeric(CEP $bÂ.1000..cal.mol.1.Â.K.1 ) ;
    c<- as.numeric(CEP $cÂ.10.5..cal.mol.1.Â.K.1) ;
...
GasGibbsFreeEnergyUncertainty<-function(Gas, Tk)       #//Calculates uncertainties of gas species
{
 ...
 # as.numeric(SD..cal.mol.1);#[4]
  #as.numeric(CEP$SD..cal.mol.1.Â.K.1);#[6]
  #as.numeric(CEP$SD..cal.mol.1.Â.K.1.1);#[10]
  ...
  if(is.na( CEP$Name == Gas))
   {
    ...
    if (!(is.na( as.numeric(CEP$S..cal.mol.1.Â.K.1)) ))
      {
      oS<- as.numeric(CEP$S..cal.mol.1.Â.K.1);#[6]
      }
      else
      {
        oS<-0;
    ...
    if (!(is.na(CEP $a..cal.mol.1.Â.K.1 ) ))
      {
      oa<- as.numeric(CEP $a..cal.mol.1.Â.K.1 );
       }
      else
      {
        oa<-0;
    ...
    if (!(is.na(CEP $bÂ.1000..cal.mol.1.Â.K.1 ) ))
      {
      ob<- as.numeric(CEP $bÂ.1000..cal.mol.1.Â.K.1 ) ;
      } 
      else 
      {
        ob<-0;
    ...
    if (!(is.na(CEP $cÂ.10.5..cal.mol.1.Â.K.1) ))
     {
      oc<- as.numeric(CEP $cÂ.10.5..cal.mol.1.Â.K.1); 
     }
      else
     {
        oc<-0;
