...
#imported functions
 ...
 MinGibbsFreeEnergyUncertainty <- function(Min, Tk,T0,CEP_2,row)    #Calculates uncertainties of minerals
 {
   ...
   ob<-0;
    ...
    oS<-as.numeric(CEP_2$S..cal.mol.1.Â.K.1[row]);
  oV<-as.numeric(CEP_2$SD..cal.mol.1.Â.K.1[row]);
   oa<-as.numeric(CEP_2$V..cm3.mol.1[row]);
   ob<-as.numeric(CEP_2$SD..cm3.mol.1[row]);
   oc<-as.numeric(CEP_2$a..cal.mol.1.Â.K.1[row] );
 ...
 SolvFunc <- function(p, TC, Pbar,CEP_2)           #Solvent function (g), Shock et al , 1992; Sue et al , 2002 - valid at Ï>=0.20 and <=1, Ï>=0.35 - Shock et al., 1992 (p. 809, Fig. 6)
 {
   g<-0;
   ag<-0;
   bg<-0;
 ...
 {  
   ...
   V<- as.numeric(CEP_2$S..cal.mol.1.Â.K.1[row]);
   
   a<- as.numeric(CEP_2$SD..cal.mol.1.Â.K.1[row]);
 ...
 #other functions
 waterGibbsFreeEnergy<- function(Tk,Pbar,p,CEP_2,row) #Calculates the Gibbs energy ofwater from triple point Ptr and Ttr T0 10 kbar and 1273.15 Â°k (Helgeson and kirkham, 1974; Hill, 1990; Johnson and NorT0n, 1991; Han, 2008)
 { 
 ...
 AqGibbsFreeEnergy<- function(Aq,Tk, Pbar, gH2O,  eTP,CEP_2,row)     #//Calculates Gibbs Free energies of aqueous species
 {
   ...
   psi = 2600;  #{bar}  //Solvent constant, Shock et al., 1992 (Appendix A-B, pp. 819-820)
    ...
    S<- as.numeric(CEP_2$S..cal.mol.1.Â.K.1[[row]]) ;
      
  

     a1<- as.numeric(CEP_2$a..cal.mol.1.Â.K.1[[row]]) ;
  
     a2<- as.numeric(CEP_2$SD..cal.mol.1.Â.K.1[row])  ;
  
    a3<- as.numeric(CEP_2$cÂ.10.5..cal.mol.1.Â.K.1[row]) ;
 ...
 {  
   ...
   psi = 2600;  #{bar}  //Solvent constant, Shock et al., 1992 (Appendix A-B, pp. 819-820)
       ...
       oa1<- as.numeric(CEP_2$a..cal.mol.1.Â.K.1[row] );
  
       oa2<- as.numeric(CEP_2$bÂ.1000..cal.mol.1.Â.K.1[row] ) ;
    
  
       oa3<- as.numeric(CEP_2$cÂ.10.5..cal.mol.1.Â.K.1[row]) ;
 ...
 GasGibbsFreeEnergy<-function(Gas,Tk,Pbar,CEP_2,row)                #//Calculates Gibbs Free energies of gas species
 {
   ...
   G<- as.numeric(CEP_2$G..cal.mol.1[row]) ;
   S<- as.numeric(CEP_2$S..cal.mol.1.Â.K.1[row]) ;
   a<- as.numeric(CEP_2 $a..cal.mol.1.Â.K.1[row] ) ;
   b<- as.numeric(CEP_2 $bÂ.1000..cal.mol.1.Â.K.1 [row]) ;
   c<- as.numeric(CEP_2 $cÂ.10.5..cal.mol.1.Â.K.1[row]) ;
 ...
 {            
   ...
   #as.numeric(CEP$SD.10.5..cal.mol.1..K.1);#[14]
       ...
       oS<- as.numeric(CEP_2$S..cal.mol.1.Â.K.1[row]);#[6]
     
    
       oa<- as.numeric(CEP_2 $a..cal.mol.1.Â.K.1[row] );
    
    
       ob<- as.numeric(CEP_2 $bÂ.1000..cal.mol.1.Â.K.1[row] ) ;
    
       oc<- as.numeric(CEP_2 $cÂ.10.5..cal.mol.1.Â.K.1[row]); 
